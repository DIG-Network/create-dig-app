...
export function nextSteps({ slug, meta }) {
  ...
  // The static-site template has no dev server (its "build" just copies src/ → public/), so skip the
  // `npm run dev` step for it; every other template runs a real dev server.
  if (meta.buildCommand && meta.name !== "static-site") {
