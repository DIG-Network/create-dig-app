...
export { TEMPLATES, UnknownTemplateError, resolveTemplate, templateNames };
 ...
 * @returns {{ appName: string, template: string, targetDir: string, nextSteps: string[] }}
 */
export function scaffold({ appName, template, targetDir }) {
  ...
  const src = join(TEMPLATES_ROOT, meta.name);
  if (!existsSync(src)) {
    throw new Error(`template files for "${meta.name}" are missing at ${src}`);
  }
